# instemmingservice chart

The Helm chart installs instemmingservice and by default the following dependencies using subcharts:
...
```bash
helm repo add instemmingservice https://raw.githubusercontent.com/ConductionNL/instemmingservice/master/api/helm/
helm repo update
```
...
```bash
helm install my-instemmingservice ConductionNL/instemmingservice --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `instemmingservice` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `instemmingservice` |
